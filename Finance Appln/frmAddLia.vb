...
Public Class frmAddLia
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        If (TextBox1.Text <> "" And ComboBox1.SelectedIndex <> -1 And ComboBox2.SelectedIndex <> -1 And TextBox2.Text <> "" And TextBox3.Text <> "") Then
            cn.Open()
            cmd.Connection = cn
            cmd.CommandText = "insert into Liablities values('" + TextBox1.Text + "','" + ComboBox1.SelectedItem.ToString() + "','" + ComboBox2.SelectedItem.ToString() + "','" + DateTimePicker1.Value.ToShortDateString() + "','" + DateTimePicker2.Value.ToShortDateString() + "','" + TextBox2.Text + "','" + TextBox3.Text + "')"
            cmd.ExecuteNonQuery()
            cmd.Dispose()
            cn.Close()
    ...
    Private Sub frmAddLia_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub Auto_Gen()
        ...
        cn.Open()
        cmd.Connection = cn
        cmd.CommandText = "select count(*) from Liablities"
        rs = cmd.ExecuteReader()
        While (rs.Read())
        ...
        If (cnt > 0) Then
            ...
            cn.Open()
            cmd.Connection = cn
            cmd.CommandText = "select max(LID) from Liablities"
            rs = cmd.ExecuteReader()
            While (rs.Read())
